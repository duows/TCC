...
O padrão de memória suportado por uma plataforma é determinado, em última instância, pelo controlador de memória
integrado ao próprio processador. Nas plataformas AM4 e AM5, adotadas neste trabalho, cada processador suporta um
único padrão, DDR4 e DDR5 respectivamente, e as placas-mãe compatíveis reproduzem essa restrição na geometria de
seus \textit{slots}. Nessas plataformas, portanto, a dependência da memória em relação ao processador manifesta-se
integralmente por intermédio da placa-mãe, o que permite modelá-la como uma restrição binária entre Placa-mãe e RAM,
preservando a cadeia de dependências transitivas do grafo de restrições.

Essa delimitação é intencional e não se generaliza a toda plataforma. Existem processadores cujo controlador de
memória admite mais de um padrão, situação em que placas-mãe distintas de um mesmo soquete oferecem padrões
diferentes para o mesmo processador. Nesses casos, a modelagem fiel exigiria uma restrição adicional e direta entre
CPU e RAM, que introduziria um ciclo no grafo de restrições e demandaria as considerações discutidas na
Subseção~\ref{subsec:tratabilidade}. A ampliação da base de conhecimento a plataformas com essa característica é
registrada entre os trabalhos futuros no Capítulo~\ref{cap:12}.
...
expresso em watts. A fonte de alimentação deve fornecer capacidade suficiente para atender à demanda energética dos componentes. O
sobredimensionamento em relação ao consumo calculado constitui prática recomendada pelos fabricantes, que incorporam reserva de
potência às próprias ferramentas de dimensionamento, destinada a cobrir picos de carga, eventual substituição de componentes e a
estabilidade da operação \cite{SEASONIC2026}. Não há, contudo, norma técnica que fixe um percentual específico de folga, de modo que
o fator adotado neste trabalho é declarado explicitamente como heurística de projeto, em conformidade com o requisito RNF-09 e
conforme detalhado na Subseção~\ref{sub:impl-avaliador}. Embora o dimensionamento físico de uma fonte considere o
consumo conjunto de todos os componentes, a verificação de compatibilidade adotada neste trabalho modela essa restrição de forma binária,
confrontando individualmente a demanda dos componentes de maior consumo, o processador e a placa de vídeo, com a capacidade da fonte.
Essa decisão de modelagem preserva a natureza binária do grafo de restrições e é detalhada no Capítulo~\ref{cap:05}. Uma fonte subdimensionada
provoca instabilidade do sistema, reinicializações inesperadas e, em casos extremos, danos permanentes aos componentes. Essa restrição difere
das anteriores por ser de natureza quantitativa: não se trata de uma incompatibilidade binária, mas de uma relação de desigualdade entre a
capacidade da fonte e a demanda de cada componente de maior consumo.
...
conjunto de modelos comercialmente disponíveis em cada categoria constitui o domínio dessas variáveis. Essa correspondência torna o CSP não apenas uma metáfora conveniente, mas a
...
Essa estrutura de grafo de restrições é o objeto central sobre o qual os algoritmos de busca e propagação
operam.
...
Pares como GPU e RAM ou CPU e GPU não possuem restrições físicas diretas e, portanto,
...
Dentro dessa estrutura, a classe P reúne os problemas para os quais existem algoritmos determinísticos
de tempo polinomial, considerados eficientes e tratáveis mesmo para entradas de grande escala.
A classe NP, por sua vez, abrange os problemas cujas soluções propostas podem ser verificados em tempo polinomial
...
Essa natureza NP-completa condiciona a arquitetura de um sistema de verificação de compatibilidade que pretenda operar sobre catálogos de tamanho arbitrário e conjuntos de restrições extensíveis. A primeira limitação é de ordem temporal,
...
lógicas complexas por árvores de decisão pode exigir espaço de tamanho exponencial \cite{Jukna1999}. Diante
...
\subsection{Tratabilidade Estrutural da Instância Modelada}
\label{subsec:tratabilidade}

A classificação do CSP como NP-completo, discutida na seção anterior,
refere-se à classe do problema em seu caso geral e fundamenta a decisão de
projeto por um motor de propagação genérico em detrimento de regras
condicionais codificadas. Cabe, contudo, distinguir a dificuldade da classe
da dificuldade da instância concreta modelada neste trabalho.

O grafo de restrições apresentado na Subseção~\ref{subsec:grafo-restricoes} possui cinco vértices e
quatro arestas, dispostas na sequência RAM--Placa-mãe--CPU--Fonte--GPU.
Trata-se, portanto, de um grafo conexo e acíclico, ou seja, uma árvore. Essa
propriedade estrutural não é acidental, mas decorre da topologia física das
dependências entre os componentes considerados: cada restrição vincula
exatamente um par de categorias, e nenhum par é alcançado por dois caminhos
distintos de dependência.

A literatura estabelece que CSPs cujo grafo de restrições é acíclico constituem uma subclasse tratável.
\textcite{Freuder1982} demonstra que, em redes com essa estrutura, a arco-consistência é condição suficiente para
busca livre de retrocesso, resultado que permite resolver o problema em tempo $O(nd^2)$ por meio de
arco-consistência direcionada sobre uma ordenação topológica das variáveis \cite{Russell2010,Dechter2003}. Duas
consequências decorrem desse resultado e são relevantes ao sistema proposto. A primeira é que, uma vez estabelecida
a arco-consistência sobre domínios não vazios, a existência de solução está assegurada, de modo que o motor decide a
satisfatibilidade da rede sem enumerar o espaço de configurações completas. A segunda é que todo valor remanescente
participa de ao menos uma solução completa, o que, aplicado ao domínio de \textit{hardware}, significa que nenhum
componente exibido ao usuário como disponível conduz a uma configuração impossível de completar: a interface está
estruturalmente livre de becos sem saída.

Essa observação delimita com precisão o alcance da contribuição deste
trabalho. A escolha do AC-3 como motor de inferência não se justifica pela
intratabilidade da instância de hardware, que é estruturalmente favorável,
mas por duas razões distintas. A primeira é a generalidade: o motor opera
sobre qualquer grafo de restrições binárias fornecido pela base de
conhecimento, sem pressupor topologia acíclica, o que preserva sua
aplicabilidade à ampliação do catálogo prevista como trabalho futuro. A
segunda é a explicabilidade: a propagação de restrições produz, no instante
da poda, a informação necessária à geração das justificativas educativas,
propriedade que a busca exaustiva não oferece.

Registra-se, ainda, que a introdução de novas categorias de componentes
tende a romper a aciclicidade do grafo. A inclusão de um cooler, por
exemplo, estabeleceria restrições simultâneas com a CPU, por soquete e por
capacidade térmica, e com a memória RAM, por altura física, fechando o ciclo
CPU--Cooler--RAM--Placa-mãe--CPU. Nesse cenário, a arco-consistência deixa
de ser suficiente para decidir a satisfatibilidade e passa a operar como
filtro, demandando sua combinação com busca sistemática. Essa ampliação é
discutida entre os trabalhos futuros no Capítulo~\ref{cap:12}.
...
sem propagar as consequências dessas remoções para variáveis mais distantes no grafo. A forma mais completa
de propagação, que estende esse raciocínio a toda a rede e garante consistência global, é o Algoritmo AC-3 \cite{Mackworth1977,Russell2010}.
...
em que o usuário seleciona um processador com soquete AM5. A Figura\ref{fig:execucao_ac3}
...
especialista humano em um domínio restrito e bem definido \cite{Russell2010}. Diferentemente de sistemas
de propósito geral, um SE concentra seu conhecimento em um domínio específico e é capaz de resolver
problemas nesse domínio com nível de competência comparável ao de um especialista humano \cite{Todd1992}.
...
\textit{explanation facility}, é o que distingue fundamentalmente um SE de um mecanismo
...
apresentando resultados sem elaboração. Um SE com \textit{explanation facility} responde também à pergunta
"por que as demais opções são inválidas?", articulando a regra específica que foi violada.

No contexto do sistema proposto, essa capacidade se manifesta da seguinte forma: ao bloquear a seleção de um módulo DDR4 após
...
compatível com o processador selecionado suporta exclusivamente DDR5 e este módulo é DDR4". Essa explicação
...
raciocínio ao usuário, permaneceu latente por décadas e ressurgiu com força renovada no contexto da Inteligência Artificial (IA)
...
técnica de pós-processamento, e sim pela própria natureza simbólica do modelo. Isso posiciona o sistema proposto em alinhamento com
...
são bloqueados, mas as razões físicas e técnicas subjacentes a cada bloqueio \cite{Todd1992}.
...
cuja interface é projetada com objetivo pedagógico explícito. A distinção em relação a um SE convencional reside na forma como
...
um SE educacional articula a cadeia de raciocínio que levou àquela conclusão em linguagem acessível ao usuário \cite{Clancey1983}.
...
via propagação AC-3, o sistema não se limita a bloquear o componente incompatível, ele apresenta ao usuário uma justificativa explícita
...
uma CPU com plataforma AM5, o sistema exibe: "Este módulo de memória é incompatível: a placa-mãe compatível com o processador selecionado
...
Essa abordagem é pedagogicamente coerente com o público-alvo do sistema, o usuário leigo que realiza uma única sessão de configuração,
...
pode ser redesenhada sem alterar o motor CSP, e o motor CSP pode ser otimizado sem alterar a interface.

O princípio \textit{stateless} (sem estado), que estabelece que cada requisição enviada pelo cliente ao servidor deve conter
...
Figura~\ref{fig:diagrama_arquitetura}.
...
A camada de dados, representada no painel esquerdo da figura, armazena
...
A camada de lógica de negócio, representada no painel central, é o
...
A camada de apresentação, representada no painel direito, é a interface
...
selecionar componentes de um catálogo extenso e verifica automaticamente incompatibilidades entre as peças
...
baseada em CSP superam abordagens baseadas em regras estáticas tanto em expressividade quanto em eficiência de
...
trabalho. Em relação às plataformas comerciais, o diferencial está na \textit{explanation facility}: enquanto PC Part Picker
