...
export function avaliarRestricao(
  ...
  switch (restricao.operador) {
    ...
    default: {
      ...
      throw new Error(`Operador de restricao nao suportado: ${_exaustivo}`);
    }
  }
}
\end{lstlisting}
...
A aplicação do operador é realizada por meio de uma estrutura de seleção exaustiva sobre o
tipo enumerado \texttt{OperadorRestricao}, cuja completude é verificada pelo compilador: a
inclusão de um novo operador na base de conhecimento sem o correspondente tratamento no
avaliador impede a compilação do projeto, em vez de produzir comportamento silencioso em
tempo de execução. Essa propriedade é relevante ao requisito RNF-07, pois assegura que a
expansibilidade do motor por meio de dados permaneça acompanhada de verificação estática do
avaliador que os interpreta.
...
A terceira limitação decorre do tratamento de características ausentes. O avaliador genérico
considera satisfeita toda restrição cujos operandos não estejam cadastrados para o par de
componentes avaliado, decisão que evita bloqueios espúrios decorrentes de cadastro
incompleto, mas que admite, como contrapartida, que uma característica omitida na base de
conhecimento resulte na liberação de um componente efetivamente incompatível. Trata-se de
manifestação particular da dependência entre a qualidade da inferência e a qualidade da base
de conhecimento já apontada. A mitigação estrutural, por meio de validação de completude no
momento do cadastro que assegure que toda categoria participante de uma restrição possua as
características correspondentes registradas para todos os seus componentes, é identificada
como trabalho futuro.
