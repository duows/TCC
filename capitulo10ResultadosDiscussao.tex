...
\input{imagens/output/tabela_regime1.tex}
...
maior cenário, com \num{6125} checagens em $d=500$. Observa-se, contudo, que o espaço residual
...
Essa ausência de poda não constitui ineficiência do motor. Ela decorre da definição de
consistência de arco apresentada na Seção~\ref{sec:ac3}, combinada com a construção dos
cenários descrita na Subseção~\ref{subsec:aval-desempenho}: os campos categóricos são gerados
por rodízio sobre as plataformas suportadas e os campos quantitativos são amostrados em faixas
que asseguram suporte mútuo entre demanda e capacidade, de modo que todo valor de uma variável
encontra suporte em ao menos um valor de cada variável vizinha. A rede já se apresenta,
portanto, arco-consistente, e nenhum valor se torna elegível à remoção. O motor executa,
verifica a consistência e encerra sem podar.

A economia observada neste regime é, portanto, de natureza algorítmica e não de redução de
domínio. Cabe observar que, sobre o grafo acíclico caracterizado na
Subseção~\ref{subsec:tratabilidade}, o resultado é mais forte do que a arco-consistência
isoladamente asseguraria: em redes acíclicas arco-consistentes cujos domínios permaneçam não
vazios, garante-se a existência de solução \cite{Freuder1982}. O AC-3 decide, assim, a
satisfatibilidade da rede com \num{608} avaliações de restrição em $d = 50$, ao passo que a
força bruta não conclui a enumeração sequer dentro do teto estabelecido. A
...
\input{imagens/output/tabela_regime2.tex}
...
\num{812500} configurações decompõe-se no produto entre a CPU fixada, as $13$ placas-mãe de
...
que no regime irrestrito, alcançando \num{33878} checagens em $d=500$ contra \num{6125} no caso
...
\input{imagens/output/tabela_comparativa.tex}
...
para \num{812500} configurações. Nesse cenário, a força bruta passa a concluir a enumeração e
identifica \num{749125} configurações válidas, valor coerente com o espaço residual reportado
